...
keywords: [named entity recognition, volunteered geographic information, natural language processing, place name extraction]
doi: https://doi.org/10.1080/13658816.2022.2133125

categories: [open-access, figshare, github, peer-reviewed, journal]
image: "figs/figure2.jpg"
date: 2022-10-02
other-links: [{href: https://github.com/cjber/ger-wiki, icon: github, rel: icon, text: Github Repository}, {href: https://figshare.com/articles/journal_contribution/Transformer_based_named_entity_recognition_for_place_name_extraction_from_unstructured_text/13415255, icon: database, rel: icon, text: FigShare Repository}]
subtitle: "{{< ai open-access >}} {{< ai figshare >}} {{< ai doi >}}"
description: "This published research article builds a custom transformer-based named entity recognition model (NER), evaluating the performance of this model against existing solutions for geoparsing geographic place names from text. This project uses the AllenNLP python library, containerised using Docker for reproducibility."

citation:
  type: article-journal
  container-title: "International Journal of Geographical Information Science"
  doi: "10.1080/13658816.2022.2133125"
  url: https://www.tandfonline.com/doi/full/10.1080/13658816.2022.2133125
---
...
@sec-trans-review outlines the research and concepts associated with geography in NLP, considering its relation to the new forms of geographic data present online, the techniques in natural language processing that explicitly deal with geography, and the developments in NLP that have enabled higher accuracy with limited labelled data. @sec-trans-methodology presents the workflow undertaken for the models constructed in this paper, as well as the data collection and analysis of the entities extracted. The performance of each NER model is then presented in @sec-trans-results-discussion and evaluated against pre-built solutions using a corpus of labelled test data. Place names are extracted using the model for the entire Wikipedia corpus, and compared against GeoNames, identifying names that are not present, discussing the reasons they may be found within Wikipedia articles, but not in an explicitly geographic gazetteer.

## Literature review {#sec-trans-review}
...
![Overview of the model processing pipeline](./figs/figure1.pdf){#fig-workflow width=80%}
...
Significant results that reject this null hypothesis were indicated by $p<0.05$ and are shown on @tbl-eval.
...
## Results & discussion {#sec-trans-results-discussion}
...
\end{table}
```
Geographic entity recognition mean (±SD) performance metrics over 3 runs of annotated Wikipedia test data subsets. Pre-built NER models are shown in italics. Bold values indicate statistically significant F1 scores of fine-tuned models in relation to `Stanza` (Paired t-tests $p<0.05$).
:::
